        ...
        public void B() {
            ...
            var query = DbContext.Set<StaffSalesPercentile>()
                .Query((VwStaffSales sales, Staffs staffs, StaffSalesPercentile alias) => {
                    ...
                    var cumeDist = AggregateBy(CUME_DIST()).OVER(PARTITION(BY(sales.Year)).ORDER(BY(sales.NetSales).DESC));
            ...
            foreach (var percentile in query.Take(5))
                Console.WriteLine((percentile.FullName, percentile.NetSales,percentile.Year, percentile.Percentile));
            #endregion
